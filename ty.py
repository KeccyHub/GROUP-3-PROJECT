...
def play_video_with_audio(video_path, audio_path):
    ...
    control_frame = tk.Frame(g3, bg="#636363", width=1, height=1)
    control_frame.pack(fill="x", side=tk.BOTTOM, padx=3, pady=2)
    ...
    def update_slide(event):
        val.set(video_player.current_duration())
        slider.set(video_player.current_duration())
        pygame.mixer.music.play(start=video_player.current_duration())
    ...
    def skip(value:int):
        global new_position, current_position
    ...
    back_btn = tk.Button(control_frame, image=backward, command= lambda: skip(-5))
    back_btn.grid(row=1, column=0, padx=10)

    play_image = tk.PhotoImage(file="play.png")
    play_btn = tk.Button(control_frame, image=play_image, command=play_video)
    play_btn.grid(row=1, column=1, padx=20)
    ...
    forward_btn.grid(row=1, column=3, padx=20)
    ...
    sound_btn.grid(row=1, column=4, padx=20)
    ...
    volume_down_btn.grid(row=1, column=5, padx=20)
    
    vol_up = tk.PhotoImage(file="increase_volume.png")
    volume_up_btn = tk.Button(control_frame, image=vol_up, command=increase_volume)
    volume_up_btn.grid(row=1, column=6, padx=20)

    quit_image = tk.PhotoImage(file="quit.png")
    quit_btn = tk.Button(control_frame, image=quit_image, command= quit_video)
    quit_btn.grid(row=1, column=7, padx=20)

    
    stop_image = tk.PhotoImage(file="stop.png")
    stop_btn = tk.Button(control_frame, image=stop_image, command=stop_video)
    stop_btn.grid(row=1, column=8, padx=20)

    end_time = tk.Label(control_frame, height=4, text=str(datetime.timedelta(seconds=0)))
    end_time.grid(row=1, column=9, pady=20)

    slider = tk.Scale(control_frame, width=35,variable=val, from_=0, to=0, orient="horizontal", command=seek)
    slider.grid(row=1, column=10, columnspan=20, padx=5, sticky="ew")
